...
namespace ApplicationLayer.Features.Employees.Commands
{
    public class DeactivateEmployeeCommandHandler
        : IRequestHandler<DeactivateEmployeeCommand, EmployeeDto>
    {
        ...
        public async Task<EmployeeDto> Handle(DeactivateEmployeeCommand request, CancellationToken ct)
        {
            ...
            if (_currentUser.Role != UserRole.ShopManager)
                throw new UnauthorizedAccessException("Only shop managers can deactivate employees.");
            ...
            await _auditRepo.LogAsync(
                ...
                employee.Id,
                true,
                metadata: $"Employee deactivated from branch {employee.BranchId}"
            );
